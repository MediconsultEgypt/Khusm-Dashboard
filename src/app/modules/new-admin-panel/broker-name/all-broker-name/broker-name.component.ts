...
import { brokerName, brokerNameResponse } from '../broker-name.model';


@Component({
    selector: 'app-broker-name',
    templateUrl: './broker-name.component.html',
    styleUrls: ['./broker-name.component.css']
})
export class BrokerNameComponent implements OnInit {
  @ViewChild('closeModal') closeModal: ElementRef

  // Decorator
  @ViewChild(DatatableComponent) table: DatatableComponent;

  brokerNameData: brokerName[];
  selectedItems: brokerName[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _BrokerNameService: BrokerNameService
    ) {
  ...
  exportToPdf() {
    const data = this.brokerNameData;
  ...
  // *export to excel
  exportExcel() {
    const data = document.getElementById("brokerNameData");
    const ws: xlsx.WorkSheet = xlsx.utils.table_to_sheet(data);
    const wb: xlsx.WorkBook = xlsx.utils.book_new();
    xlsx.utils.book_append_sheet(wb, ws, 'Sheet1');
    xlsx.writeFile(wb, 'broker_name.xlsx');
  }
  ...
  // get broker name
  getBrokerName(page: number) {
    this._BrokerNameService.get_broker_name(page)
      .subscribe((res: brokerNameResponse) => {
        this.brokerNameData = res.results;
  ...
  onPageChange(event: any): void {
    this.getBrokerName(event.page +1);
  }
