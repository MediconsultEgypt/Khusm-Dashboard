...
import { prov_rate, providerResponse } from '../provider-rate.model';
import * as FileSaver from 'file-saver';

@Component({
    selector: 'app-all-provider-rate',
    templateUrl: './all-provider-rate.component.html',
    styleUrls: ['./all-provider-rate.component.css']
})
export class ProviderRatesComponent implements OnInit {
  ...
  rates: prov_rate[];
  selectedProviderBranches: prov_rate[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _ProviderRateService: ProviderRateService
    ) {
  ...
  // *export to excel
//   exportExcel() {
//     const data = document.getElementById("rates");
//     const ws: xlsx.WorkSheet = xlsx.utils.table_to_sheet(data);
//     const wb: xlsx.WorkBook = xlsx.utils.book_new();
//     xlsx.utils.book_append_sheet(wb, ws, 'Sheet1');
//     xlsx.writeFile(wb, 'prov_rate.xlsx');
// }

exportExcel() {
  import('xlsx').then((xlsx) => {
    const worksheet = xlsx.utils.json_to_sheet(this.rates);
    const workbook = { Sheets: { data: worksheet }, SheetNames: ['data'] };
    const excelBuffer: any = xlsx.write(workbook, {
      bookType: 'xlsx',
      type: 'array',
    });
    this.saveAsExcelFile(excelBuffer, 'provider_rates');
  });
}
...
saveAsExcelFile(buffer: any, fileName: string): void {
  ...
  // get rates
  getAll(page: number) {
    this._ProviderRateService.get_all(page)
