...
import { packageResponse, packages_promo_code } from '../package-promo-code.model';
import * as FileSaver from 'file-saver';

@Component({
    selector: 'app-package-promo-code',
    templateUrl: './package-promo-code.component.html',
    styleUrls: ['./package-promo-code.component.css']
})
export class PackagePromoCodeComponent implements OnInit {
  @ViewChild('closeModal') closeModal: ElementRef

  // Decorator
  @ViewChild(DatatableComponent) table: DatatableComponent;

  packages_promo_code: packages_promo_code[];
  selectedProviderBranches: packages_promo_code[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _PackagePromoCodeService: PackagePromoCodeService
    ) {
  ...
  ngOnInit(): void {
    ...
    //   { field: this.serviceItemName, header: 'NAME' },
      ...
      { field: 'used_times', header: 'Used times' },
      { field: 'active', header: 'Active' },
      { field: 'percent_discount', header: 'Percent discount' },
      { field: 'generate_more', header: 'Generate more' },
      { field: 'number_of_promocodes', header: 'Number of promocodes' },
    ];
  ...
  exportToPdf() {
    const data = this.packages_promo_code;
  ...
  // *export to excel
//   exportExcel() {
//     const data = document.getElementById("packages_promo_code");
//     const ws: xlsx.WorkSheet = xlsx.utils.table_to_sheet(data);
//     const wb: xlsx.WorkBook = xlsx.utils.book_new();
//     xlsx.utils.book_append_sheet(wb, ws, 'Sheet1');
//     xlsx.writeFile(wb, 'packages_promo_code.xlsx');
// }

exportExcel() {
  import('xlsx').then((xlsx) => {
    const worksheet = xlsx.utils.json_to_sheet(this.packages_promo_code);
    const workbook = { Sheets: { data: worksheet }, SheetNames: ['data'] };
    const excelBuffer: any = xlsx.write(workbook, {
      bookType: 'xlsx',
      type: 'array',
    });
    this.saveAsExcelFile(excelBuffer, 'package_promo_code');
  });
}
...
saveAsExcelFile(buffer: any, fileName: string): void {
  ...
  // get packages_promo_code
  getAll(page: number) {
    this._PackagePromoCodeService.get_all(page)
      .subscribe((res: packageResponse) => {
        this.packages_promo_code = res.results;
