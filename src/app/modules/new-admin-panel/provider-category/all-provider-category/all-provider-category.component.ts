...
import { providerCategory, providerCategoryResponse } from '../all-provider-category.model';

@Component({
    selector: 'app-all-provider-category',
    templateUrl: './all-provider-category.component.html',
    styleUrls: ['./all-provider-category.component.css']
})
export class ProviderCategoryComponent implements OnInit {
  @ViewChild('closeModal') closeModal: ElementRef

  // Decorator
  @ViewChild(DatatableComponent) table: DatatableComponent;

  providerCategoryData: providerCategory[];
  selectedItems: providerCategory[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _ProviderCategoryService: ProviderCategoryService
    ) {
  ...
  exportToPdf() {
    const data = this.providerCategoryData;
  ...
  // *export to excel
  exportExcel() {
    const data = document.getElementById("providerCategoryData");
  ...
  getAll(page: number) {
    this._ProviderCategoryService.getAll(page)
      .subscribe((res: providerCategoryResponse) => {
        this.providerCategoryData = res.results;
