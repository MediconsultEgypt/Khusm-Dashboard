...
import { Groups, User_permissions } from '../user-all-method.model';

@Component({
  selector: 'app-getById-user-method',
  templateUrl: './getById-user-method.component.html',
  styleUrls: ['./getById-user-method.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class GetByIdUserMethodComponent implements OnInit, OnDestroy {
  ...
  // datatable 2
  selectedUserPermissions: User_permissions[] | null = [];
  branch_cols: any[] = [];
  ...
  constructor(private router: Router, private _UserMethodService: UserMethodService) {
   ...
   */
  ngOnInit(): void {
    // this._UserMethodService.onUserViewChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
    //   this.data = response;
    // });
    ...
    //     { field: 'user_method', header: 'user_method' },
    ...
    //     { field: 'provider_branch_location', header: 'Location' },
  ...
  // *export to excel
  exportExcel() {
    const data = document.getElementById("user_method");
  ...
  getById(id: number ) {
    this._UserMethodService.getByID(id)
      .then((res) => {
        this.user_method = res;
        // this.Groups = this.user_method.Groups;
        // this.User_permissions = this.user_method.User_permissions;
        console.log(this.user_method)
      })
      .catch((error) => {
        // Handle the error
        console.error(error);
