...
import { providerProduct, providerProductResponse } from '../provider-product.model';
...
export class ProviderProductComponent implements OnInit {
  ...
  providerProductData: providerProduct[];
  selectedItems: providerProduct[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _ProviderProductService: ProviderProductService
    ) {
  ...
  exportToPdf() {
    const data = this.providerProductData;
  ...
  // *export to excel
  // exportExcel() {
  //   const data = document.getElementById("providerProductData");
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      const worksheet = xlsx.utils.json_to_sheet(this.providerProductData);
      const workbook = { Sheets: { data: worksheet }, SheetNames: ['data'] };
      const excelBuffer: any = xlsx.write(workbook, {
        bookType: 'xlsx',
        type: 'array',
      });
      this.saveAsExcelFile(excelBuffer, 'provider_product');
    });
  }
  ...
  getAll(page: number) {
    this._ProviderProductService.getAll(page)
      .subscribe((res: providerProductResponse) => {
        this.providerProductData = res.results;
