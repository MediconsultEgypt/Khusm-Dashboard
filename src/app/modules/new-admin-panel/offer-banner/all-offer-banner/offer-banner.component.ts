...
import { OfferBannerResponse, offerBannerData } from '../offer-banner.model';
...
export class OfferBannerComponent implements OnInit {
  ...
  selectedProviderBranches: offerBannerData[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _OfferBannerService: OfferBannerService
    ) {
  ...
  set selectedColumns(val: any[]) {
    // *restore original offer-banner
    this._selectedColumns = this.cols.filter((col) => val.includes(col));
  }

  // Export table data to PDF
  exportToPdf() {
    const data = this.offerBannerData;
  ...
  // *export to excel
  exportExcel() {
    const data = document.getElementById("offerBannerData");
    const ws: xlsx.WorkSheet = xlsx.utils.table_to_sheet(data);
    const wb: xlsx.WorkBook = xlsx.utils.book_new();
    xlsx.utils.book_append_sheet(wb, ws, 'Sheet1');
    xlsx.writeFile(wb, 'offerBannerData.xlsx');
}
  ...
  // get offerBannerData
  getAll(page: number) {
    this._OfferBannerService.get_all(page)
      .subscribe((res: OfferBannerResponse) => {
        this.offerBannerData = res.results;
