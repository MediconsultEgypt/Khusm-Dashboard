...
import { packages, provResponse } from '../package.model';
...
export class PackageComponent implements OnInit {
  ...
  packages: packages[];
  selectedProviderBranches: packages[] | null = [];
  cols: any[] = [];
  ...
  // *export to excel
//   exportExcel() {
//     const data = document.getElementById("packages");
...
exportExcel() {
  import('xlsx').then((xlsx) => {
    ...
    this.saveAsExcelFile(excelBuffer, 'packages');
  });
}
...
saveAsExcelFile(buffer: any, fileName: string): void {
  ...
  // get packages
  getAll(page: number) {
    this._PackageService.get_all(page)
      .subscribe((res: provResponse) => {
