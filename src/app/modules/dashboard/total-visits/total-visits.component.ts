...
import { total_visits } from '../dashboard-datatables.model';


@Component({
  selector: 'app-total-visits',
  templateUrl: './total-visits.component.html',
  styleUrls: ['./total-visits.component.css']
})
export class TotalVisitsComponent implements OnInit {
  ...
  selectedItems: total_visits[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    injector: Injector,
    private _TotalVisitsService: DashboardDatatablesService
    ) {
  ...
  ngOnInit(): void {
    ...
    //   { field: this.serviceItemName, header: 'NAME' },
      ...
      { field: 'total_visits', header: 'Total visits' },
    ];
  ...
  // *export to excel
  exportExcel() {
    const data = document.getElementById("total_visits");
  ...
  // get total visits
  getAll() {
    this._TotalVisitsService.get_total_visits()
      .subscribe((res) => {
        this.total_visits = res;
      })
  }
