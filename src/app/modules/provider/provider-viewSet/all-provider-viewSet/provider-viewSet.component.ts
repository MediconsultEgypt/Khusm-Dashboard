...
import { Allprovider_viewSet, providerResponse } from '../provider-viewSet.model';
import * as FileSaver from 'file-saver';

@Component({
    selector: 'app-provider-viewSet',
    templateUrl: './provider-viewSet.component.html',
    styleUrls: ['./provider-viewSet.component.css']
})
export class ProviderviewSetListComponent implements OnInit {
  @ViewChild('closeModal') closeModal: ElementRef

  // Decorator
  @ViewChild(DatatableComponent) table: DatatableComponent;

  providers: Allprovider_viewSet[];
  selectedServiceItems: Allprovider_viewSet[] | null = [];
  cols: any[] = [];
  ...
  constructor(
    ...
    injector: Injector,
    private _ProviderViewSetListService: ProviderViewSetListService
    ) {
  ...
  ngOnInit(): void {
    ...
    this.getAll_provider_viewSet(1);
    ...
    //   { field: this.serviceItemName, header: 'NAME' },
      ...
      { field: 'min_distance', header: 'Min distance' },
      { field: 'online_order_available', header: 'Online order available' },      
      { field: 'avg_rate', header: 'Average rate' },      
      { field: 'provider_category_name', header: 'Provider category name' },
  ...
  exportExcel() {
    import('xlsx').then((xlsx) => {
      ...
      this.saveAsExcelFile(excelBuffer, 'providers_viewSet');
    });
  }
  ...
  // get providers viewSet list
  getAll_provider_viewSet(page: number) {
    this._ProviderViewSetListService.get_provider_viewSet_list(page)
  ...
  onPageChange(event: any): void {
    this.getAll_provider_viewSet(event.page +1);
  }
